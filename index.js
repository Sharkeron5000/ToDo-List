...
function load() {
  ...
  if (!todoListNow || todoListNow.length === 0) {
    ...
    todoTip.id = 'noTodo';
    todoTip.innerHTML = `Откройте меню нажав на <span>⇶</span> и выберите тему задач`;
...
function renderGroupTodo(localstorageGroup) {
  ...
  textDiv.textContent = 'Темы задач:';
  textDiv.classList.add('TextGroupList');
  groupListDiv.appendChild(textDiv);
...
function renderGroupTip() {
  ...
  group.textContent = 'Темы задач не найдены, нажмите внизу на ➕ для создания новых'
}
...
function detailRenderTodo(todoArray, numIdGroup, todoListDiv, parentTodo = []) {
  ...
  if (parentTodo.length === 0) {
    const textDiv = document.createElement('div');
    textDiv.textContent = 'Список задач:';
    textDiv.classList.add('textTodoList');
    todoListDiv.appendChild(textDiv);
  }
  ...
  todoArray.forEach((elem, index) => {
    ...
    todoLi.classList.add('mainLiTodo');
    listUl.appendChild(todoLi);
    const mainDiv = document.createElement('div');
    mainDiv.classList.add('mainTodo');
    todoLi.appendChild(mainDiv);
    ...
    mainDiv.appendChild(completeInput);
    mainDiv.appendChild(textTodoDiv);
    mainDiv.appendChild(todoLengthDiv);
    mainDiv.appendChild(additionalMenuDiv);
